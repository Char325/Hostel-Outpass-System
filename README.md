...
<ul dir="auto">
 ...
 <ul>
  ...
  Then, clone the repo. After cloning navigate to the the client side (outpass folder) and install the dependancies as mentioned in the `package.json` file and do the same for server side (server folder). 
 ...
 3. In-use: When the current date <= return date mentioned by user, it's in use. In this state all operations can be 
    performed by the student.
 4. Cancelled: An out-pass can be cancelled any time by the student. The status changes to "Cancelled".
 5. Denied: When the warden denies the out-pass (with reason), its status changes to "Denied".
 6. Expired: When the out-pass is out of use, that is, current date > retun date mentioned by student, its status becomes 
    "Expired".
